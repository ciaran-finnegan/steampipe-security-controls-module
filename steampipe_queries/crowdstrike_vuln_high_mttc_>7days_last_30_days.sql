...
WHERE 
    cve ->> 'exprt_rating' = 'HIGH' AND crowdstrike_spotlight_vulnerability.created_timestamp >= NOW() - INTERVAL '30 DAYS' AND (crowdstrike_spotlight_vulnerability.closed_timestamp - crowdstrike_spotlight_vulnerability.created_timestamp >= INTERVAL '7 DAYS' OR crowdstrike_spotlight_vulnerability.closed_timestamp - crowdstrike_spotlight_vulnerability.created_timestamp IS NULL)
